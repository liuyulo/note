...
For the other direction, define $h:cof beta->alpha$ where $h(gamma)$ is the least ordinal such that $f(h(gamma)) > g_beta (gamma)$. Since $f:alpha->beta$ is cofinal, there is some $xi in alpha$ such that $f(xi) ⩾ g_beta (gamma)$; and because $alpha$ is a limit ordinal and $f$ is strictly increasing, $f(xi+1) > g_beta (gamma)$ and thus $h$ is well-defined.
